	...
	const onFileUpload = async () => {
		if (files.length === 0) {
			...
			await axios.post(`${baseUrl}/upload/multiple`, formData).then((res) => {
				...
				setImage({ url: res.data.data.image.path, filename: res.data.data.image.filename });
				let data = [];
				for (let idx = 0; idx < res.data.data.excel.length; idx++) {
					console.log(res.data.data.excel[idx]);
					data = [...data, { header: res.data.data.excel[idx].header, data: res.data.data.excel[idx].data, x: 0, y: 0 }];
				};
				console.log(data);
				setHeader(data);
				setLoading(false);
